...
const OUT = path.join(ROOT, "report", "Tracer_Defence_Slides_v2.pptx");
...
If asked about methodology, the answer is the iterative and incremental model, with CRISP-DM for the machine learning part. That's slide 10.`);
}
...
/* ═════════════ 8 — SUMMARY OF THE WORK: DATA AND MODELS ═════════════ */
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
...
Finally the split at the bottom — trained on the earlier time steps, tested on the later ones. That's on the methodology slide, but flag it here.
...
/* ═══════════════════ 11 — IMPLEMENTATION: THE SYSTEM ═══════════════════ */
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
  heading(s, "Implementation", "A three-tier application");

  const tiers = [
    { t: "Presentation", s: "React 18 · Vite · Tailwind CSS", d: "Analyst console and administrator dashboards, light and dark themes.", c: INDIGO },
    { t: "Application", s: "Flask REST API · JWT · bcrypt", d: "Six blueprints, token authentication, role checks enforced on the server.", c: AMBER },
    { t: "Data", s: "MySQL · serialised models", d: "Eight tables holding the transaction graph, predictions and alerts, plus the trained model files.", c: RED },
  ];
  tiers.forEach((t, i) => {
    const y = 1.7 + i * 1.55;
    card(s, 0.6, y, 6.1, 1.35);
    // numbered circle, matching the motif used throughout the deck
    s.addShape(pres.ShapeType.ellipse, { x: 0.88, y: y + 0.44, w: 0.42, h: 0.42, fill: { color: t.c } });
    s.addText(String(i + 1), { x: 0.88, y: y + 0.44, w: 0.42, h: 0.42, align: "center", valign: "middle", fontFace: F, fontSize: 14, bold: true, color: WHITE, margin: 0 });
    s.addText(t.t, { x: 1.5, y: y + 0.2, w: 5.0, h: 0.3, fontFace: F, fontSize: 15, bold: true, color: TEXT, margin: 0 });
    s.addText(t.s, { x: 1.5, y: y + 0.5, w: 5.0, h: 0.26, fontFace: F, fontSize: 11, color: t.c, margin: 0 });
    s.addText(t.d, { x: 1.5, y: y + 0.76, w: 5.0, h: 0.5, fontFace: F, fontSize: 11.5, color: MUTED, lineSpacing: 15, margin: 0 });
  });

  s.addText("WHAT THE ANALYST CAN DO", { x: 7.05, y: 1.72, w: 5, h: 0.3, fontFace: F, fontSize: 11, bold: true, color: MUTED, charSpacing: 1.2, margin: 0 });
  [["Browse and search", "203,769 real transactions, filtered by label."],
   ["Score with either model", "Random Forest live, GraphSAGE from a precomputed store."],
   ["See the network", "The two-hop neighbourhood the graph model reasons over."],
   ["Read the explanation", "Each score split into the transaction's own features versus its network context."],
   ["Work the alert queue", "Add notes, resolve cases, export activity as CSV."]].forEach((f, i) => {
    const y = 2.12 + i * 0.85;
    s.addShape(pres.ShapeType.ellipse, { x: 7.05, y: y + 0.06, w: 0.2, h: 0.2, fill: { color: INDIGO } });
    s.addText(f[0], { x: 7.4, y, w: 4.6, h: 0.28, fontFace: F, fontSize: 13, bold: true, color: TEXT, margin: 0 });
    s.addText(f[1], { x: 7.4, y: y + 0.29, w: 4.7, h: 0.5, fontFace: F, fontSize: 11, color: MUTED, lineSpacing: 14, margin: 0 });
  });
  card(s, 0.6, 6.35, 6.1, 0.62, TINT);
  s.addText("Validated by 35 automated tests, all passing", { x: 0.88, y: 6.5, w: 5.6, h: 0.32, fontFace: F, fontSize: 12.5, bold: true, color: INDIGO, margin: 0 });
  chrome(s, false);
  s.addNotes(
`This is the architecture slide. Three tiers.

The presentation tier is React — that's what the analyst sees and clicks.
The application tier is a Flask REST API in Python — it checks who you are, runs the models, decides what to do.
The data tier is MySQL plus the saved model files.

Why three tiers? So each part can change without breaking the others. I could redesign the whole interface without touching the models.

On the right is what an analyst can actually do. The next three slides show these on screen.

Note the two serving strategies: Random Forest runs live on every request, while GraphSAGE scores come from a precomputed store, because running full-graph message passing per request is too slow on a laptop. If asked, that's a deliberate engineering trade-off.

Bottom left: 35 automated tests, all passing.`);
}

/* ═══════════ 12 — IMPLEMENTATION: SCORING A TRANSACTION ═══════════ */
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
...
/* ═══════════ 13 — IMPLEMENTATION: EXPLAINABILITY AND NETWORK ═══════════ */
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
...
/* ═══════════ 14 — IMPLEMENTATION: CONTINUOUS MONITORING ═══════════ */
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
